...
const MenuItemCard = ({item, index, isUsedInSidePanel}: Props) => {
    ...
    : `relative min-h-24 h-auto w-full md:w-72 rounded-xl p-2 flex flex-col items-center ${itemExists ? 'bg-blue-50 border border-blue-300' : 'bg-white'} dark:bg-gray-800 shadow-muted cursor-pointer hover:bg-blue-50 hover:shadow-lg border border-gray-200 hover:border-blue-500 ring-2 ring-transparent hover:ring-blue-200 transform transition-all duration-500`;
        ...
        <div key={index} className={cardStyle}>
            <div className="relative h-full w-full flex items-center gap-4">
                ...
                <div className="flex flex-col justify-start items-start">
                    ...
                    <p className="text-sm font-medium text-blue-500 mt-1">₱{isUsedInSidePanel ? item.menu.price : item.price}</p>
                </div>
                {isUsedInSidePanel && (
                ...
                <div className="w-full flex items-center justify-between mt-2">
                    ...
                    <div className="w-full">
                        ...
                        <div className="flex items-center justify-start gap-4">
                            <CiCircleMinus onClick={() => updateQuantity(item.menu.id, item.quantity-1)} size={30} className="text-gray-400 hover:text-blue-500 transform transition-all duration-500 cursor-pointer"/>
                            <p className="text-lg">{item.quantity}</p>
                            <CiCirclePlus onClick={() => updateQuantity(item.menu.id, item.quantity+1)} size={30} className="text-gray-400 hover:text-blue-500 transform transition-all duration-500 cursor-pointer"/>
                        </div>
                    </div>
                </div>
            )}
